...
Graphing is presentation-only. The fantasy-points graph shows the probability of finishing within a one-point bucket centered on each x-value (`x ± 0.5 FP`). Stat graphs instead show the fitted survival curve: the x-axis is the stat threshold and the y-axis is the probability of reaching or exceeding that threshold. Small hollow circles show each individual book's fair/de-vigged over probability at its exact threshold; larger consensus diamonds show the combined market anchors used to fit the distribution. Exact source-book thresholds are also marked on the x-axis. **Explain betting lines** expands the raw source prices, each line's fair P(over), and the consensus anchors for a selected player so the curve's shape can be traced back to the market evidence. None of these graph views changes Floor / Mid / Ceiling or creates a second projection model in the browser.
...
3. In **Player report**, click a player for source lines or open **Graphs**. The graph explorer has a left filter panel for graph type, position and player, plus Previous/Next controls to cycle through available metrics. Stat graphs show the fitted probability curve, individual-book probability circles, consensus anchor diamonds and exact sportsbook line ticks; use **Explain betting lines** for the underlying prices and fair probabilities.
...
- `oddsfantasy/line_provenance.py` — display-only per-book fair probabilities before consensus aggregation.
