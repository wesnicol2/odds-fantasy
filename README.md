...
A player is comparison-eligible only when the core markets for that position are usable: QB requires passing yards, passing TDs, interceptions and rushing yards; RB requires rushing yards, receiving yards and anytime TD; WR/TE require receiving yards and anytime TD. Receptions are also required when the league awards reception points. Peripheral markets such as WR rushing yards remain optional. If any core market is missing, the player stays visible with dashes, an explicit `INCOMPLETE`/`NO CORE LINES` state and the missing markets named, but is excluded from graphs, lineup optimization and start/sit comparisons. Missing coverage is unknown, never a zero-point projection.
