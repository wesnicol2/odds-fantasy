...
function graphDescription(metric, graph) {
  if (metric === 'fantasy_points') {
    return 'Each plotted point is the probability of finishing within x ± 0.5 fantasy points. Floor / Mid / Ceiling calculations are unchanged.';
  }
  if (!graph) return '';
  if (graph.kind === 'exact_count') return 'Each plotted point is the exact probability of that integer result.';
  const width = Number(graph.bucket_width) || 1;
  return `Each plotted point is the probability within a ${width}-unit bucket centered on x.`;
}
...
function renderGraphChart(explorer) {
  ...
  area.innerHTML = `<p class="chart-explainer">${escapeHtml(graphDescription(metric, sampleGraph))}</p>
