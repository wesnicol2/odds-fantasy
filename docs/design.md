...
Fantasy points and stat value are two measurements of the same week, so both use the same visual language: the Floor / Mid / Ceiling thermometer already used by the ranking table. Every thermometer marks all three percentiles, and the two players in one row share a single zero-anchored scale so the glyphs read as magnitudes rather than as a zoomed-in gap. In the comparison matrix the thermometers are drawn **vertically** and anchored to the boundary between the two player columns, with each player's numbers directly above their own bar: a shared baseline and a short distance are what make two ranges comparable at a glance, which two horizontal bars centred in separate cells are not. The ranking table keeps the horizontal glyph, where one bar per row is a dense column rather than a pair to compare. A thermometer is presentation only — it places backend percentiles on a scale and never derives a value the backend did not supply.
