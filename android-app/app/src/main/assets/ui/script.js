...
async function loadDefenses(week) {
  ...
  const url = apiUrl('/defenses', { username: val('username') || 'wesnicol', season: val('season') || '2025', week, scope: 'both', mode: getDataMode() });
  const { ok, data } = await fetchJSON(url);
...
function addLineupControls(week, source, target) {
  try {
    ...
    if (!header) return;
    const pill = (t) => `<button class="pill ${t===target?'pill-active':''}" data-t="${t}">${t.toUpperCase()}</button>`;
    const row = document.createElement('div');
    row.className = 'btn-row';
    row.innerHTML = `${pill('floor')}${pill('mid')}${pill('ceiling')}`;
    header.insertAdjacentElement('afterend', row);
    row.querySelectorAll('.pill').forEach(btn => btn.addEventListener('click', () => {
      const t = btn.getAttribute('data-t');
      if (source === 'players') {
        selectedTarget[week] = t;
        renderLineupFromPlayers(week);
      } else {
        loadLineup(week, t);
      }
